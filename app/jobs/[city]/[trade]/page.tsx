...
export default async function JobsTradePage({ params }: { params: Promise<Params> }) {
  const { city: citySlug, trade: tradeSlug } = await params
  const city = getCity(citySlug)
  const trade = getTrade(tradeSlug)
  ...
  return (
    <>
      <Hero
        ...
        subheading={`Stop hunting for bids. Let ${city.name}'s top GCs come to you. Join our free network and get matched with vetted general contractors looking for ${trade.singular.toLowerCase()}s right now.`}
        aside={
          <SubApplyForm
            trade={trade.name}
            city={cityLabel(city)}
      ...
      <section className="bg-background">
        <div className="mx-auto max-w-3xl px-4 py-14 sm:px-6 lg:py-20">
          ...
          <p className="mt-3 leading-relaxed text-muted-foreground">
            Here's what joining the {city.name} subcontractor network means for your{" "}
            {trade.singular.toLowerCase()} specifically:
          </p>
